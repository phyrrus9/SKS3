...
int pos = 81, i = 1, coins = 0, keys = 0, health = 100, atk = 0, def = 0, score = 0, deadzombies = 0, lvl = 0, completedlevels = 0, awards = 0, t = 81, dzc = 0, walls = 0, start[] = {81, 250}, firstrun = 1, boss[400], hits = 0;
string zprev[400];
...
if ((mseed == 2) || (mseed == 3))
  z = "#"; // #
if ((mseed == 4) || (mseed == 5) || (mseed == 6) || (mseed == 7) || (mseed == 8))
  z = "B"; // z
if (mseed == 9)
  z = "c"; // c
if (mseed >= 10)
...
string progress[42] = {"["," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," "," ","]"};
while (x != 400) {
int maxatk = 250 * (completedlevels / 3);
int brand;
if (lvl <= 5)
  brand = rand() % 100 + 0;
else
  brand = rand() % maxatk + 100;
boss[x] = brand;
b++;
if (b == 10)
{
...
system("clear");
cout << "Resetting Bosses... " << progress[0] << progress[1] << progress[2] << progress[3] << progress[4] << progress[5] << progress[6] << progress[7] << progress[8] << progress[9] << progress[10] << progress[11] << progress[12] << progress[13] << progress[14] << progress[15] << progress[16] << progress[17] << progress[18] << progress[19] << progress[20] << progress[21] << progress[22] << progress[23] << progress[24] << progress[25] << progress[26] << progress[27] << progress[28] << progress[29] << progress[30] << progress[31] << progress[32] << progress[33] << progress[34] << progress[35] << progress[36] << progress[37] << progress[38] << progress[39] << progress[40] << progress[41] << "[" << x / 4 << "%]" << endl;
x++;
}
}
...
boss[prev] -= (atk * 2 + score) - hits * 2;
hits++;

if (boss[prev] <= 0)
{
  grid[lvl][prev] = map[prev] = "$";
  deadzombies = deadzombies * prev;
  health = health * 2;
  dzc++;
  if (dzc >= 10)
  {
...
{
int healthnew = (boss[prev] - ((def * 10) - (atk * 5) - (score) * (hits + 1) + (boss[prev] / 4)));
health -= healthnew;
return "You were hit";
}
...
system("clear");
  ...
  cout << "Instructions!!!" << endl << "Use W A S D (lowercase) to move" << endl << "To puck up/use something (k, c, b), press 2 and then W A S D" << endl << "To attack, press 1 and then W A S D" << endl << "GOOD LUCK!!!!!!" << endl;
  ins++;
  sleep(1);
  is[yy] = "######";
  yy++;
...
resetmap("Loading");
system("clear");
printf("Welcome to Super Key Seeker 4200(boss rush mode) (build %s)\nYou are at the main menu, please select a difficulty:\n1. Easy\n2. Normal\n3. Hard\n4. Suicidal\n>", build);
int null = 1;
cin >> null;
if (null == 1)
{
